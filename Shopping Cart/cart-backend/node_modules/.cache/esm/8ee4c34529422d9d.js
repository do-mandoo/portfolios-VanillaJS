let mongoose,Joi,Cart,Product;_294‍.x([["checkObjectId",()=>checkObjectId],["Pwrite",()=>Pwrite],["Cwrite",()=>Cwrite],["Plist",()=>Plist],["Clist",()=>Clist],["remove",()=>remove],["update",()=>update]]);_294‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_294‍.w("joi",[["default",["Joi"],function(v){Joi=v}]]);_294‍.w("../../models/cart",[["default",["Cart"],function(v){Cart=v}]]);_294‍.w("../../models/product",[["default",["Product"],function(v){Product=v}]]);
...
POST /api/appPosts/mycart/:add
{
  ...
  const result = schema.validate(ctx.request.body);
  _294‍.g.console.log(result, 'result dsfdsfdsfds');
  if (result.error) {
    ctx.status = 404; // Bad Request
    ctx.body = result.error;
    return;
...
DELETE /api/mycart/delete/:id */
  ...
  try {
    await Cart.findByIdAndRemove({ _id: id }).exec();
    const cart = await Cart.find().exec();
    ctx.body = cart;
...
PATCH /api/mycart/modify/:id
{
  ...
  _294‍.g.console.log({ numberOfCart });
